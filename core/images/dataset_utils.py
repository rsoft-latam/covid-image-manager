...
def download_dataset():
    ...
    if os.path.exists(dataset_folder):
        print(f"⚠️ Dataset already exists at: {dataset_folder}")
        choice = input("Do you want to overwrite it? (y/n): ").strip().lower()
        if choice != "y":
            print("⏹️ Download cancelled.")
            return
        else:
            print("🗑️ Removing existing dataset folder...")
            import shutil
...
def show_dataset_summary():
    ...
    if not os.path.exists(dataset_path):
        print("⚠️ Dataset file not found. Please ensure 'data/covid_dataset.csv' exists.")
        return
